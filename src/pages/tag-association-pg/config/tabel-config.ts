	...
	// { label: "view", action: "view", size: "small", type: "success", class: "mx-[0px] mr-[5px] last:mr-0" },
	{
		...
		action: (proxy, row) => {
			proxy
				...
				.then(() => {
					proxy.$request(proxy.$apis.contentTypeTag, { _id: row._id }, { method:"delete" }).then((res: any) => {
						if (res.code == 200) {
							proxy.$parent.query();
							proxy.$message({
								message: res.msg,
...
export function topOptions(proxy:any):BatchButton[]{
	return [
		{ label: "创建标签", action: (selected, proxy)=>{
				console.log(proxy)
		...
		}, size: "default", type: "primary", class: "mx-[0px] mr-[5px] last:mr-0" },
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					...
					.then(() => {
						proxy.$request(proxy.$apis.contentTypeTag, { _id: _ids.join(",") },{method:'delete'}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要禁用所选的 [ ${_ids.length} ] 个标签吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.contentTypeTag, { _ids, status: false }, { method:"put"}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要启用所选的 [ ${_ids.length} ] 个标签吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.contentTypeTag, { _ids, status: true }, { method:"put"}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
