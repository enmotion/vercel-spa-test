<template>
	<div class="flex-col grow-1">
		...
		>
			<!-- side -->
			<div class="w-[180px] flex-col shrink-0 text-white bg-a-10">
				<div class="h-[50px] px-20 flex-col justify-center text-lg font-semibold bg-dark-4 italic text-white">
					...
					<!-- <div class="flex-row">
            <span
              class="w-0 grow-1 text-xs italic uppercase font-normal overflow-hidden text-ellipsis text-light-24"
            >
              system - {{ appStore.getInfo.version }}
            </span>
          </div> -->
				</div>
			...
			<div class="grow-1 flex-col">
				<div class="h-[50px] flex-row bg-gray-50 dark:bg-gray-900">
		...
		>
			<div
				class="h-[50px] flex-row items-center dark:bg-gray-800 dark:border-b-gray-700 justify-between px-[15px] border-b"
			>
				...
				>
					...
					>
						<keep-alive :include="keepAliveStore.getNames">
							<component :is="Component"
								:key="route.path"
								class="w-full h-full p-10 bg-gray-100 dark:bg-gray-900"
							/>
						</keep-alive>
					</vmo-transition>
				</router-view>
