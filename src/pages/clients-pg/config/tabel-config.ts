	...
	},
	{
		...
		action: (proxy, row) => {
			proxy
				...
				.then(() => {
					proxy.$request(proxy.$apis.clientsDelete, { uid: row.uid }).then((res: any) => {
						if (res.code == 200) {
							proxy.$parent.query();
							proxy.$message({
								message: res.msg,
		...
		}, disabled:proxy.selection.length!=0, size: "default", type: "primary", class: "mx-[0px] mr-[5px] last:mr-0" },
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					...
					.then(() => {
						proxy.$request(proxy.$apis.clientsDelete, { uid: selectedUid.join(",") }).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					...
					.then(() => {
						proxy.$request(proxy.$apis.clientsUpdateMany, { uid: selectedUid, status: false }).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					...
					.then(() => {
						proxy.$request(proxy.$apis.clientsUpdateMany, { uid: selectedUid, status: true }).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
