				...
				props: {
					...
					rules: {
						...
						validator:function(rule,value,cb){
							if(!!value){
								request(apis.clientsUniq,{nickname:value,uid:this.$formData.uid},{}).then(res=>{
									if(res?.data?.total==0){
										cb()
									}else{
										cb("该昵称已经被占用")
			...
			item: {
				...
				props: {
					...
					rules: {
						...
						validator:function(rule,value,cb){
							if(!!value){
								request(apis.clientsUniq,{username:value,uid:this.$formData.uid},{}).then(res=>{
									if(res?.data?.total==0){
										cb()
									}else{
										cb("该账号已经被占用")
				...
				props: {
					...
					rules: {
						...
						validator(rule, value, callback) {
							...
							if(!!value){
									request(apis.clientsUniq,{phone:value,uid:this.$formData.uid},{}).then(res=>{
										console.warn(res)
										if(res?.data?.total==0){
											callback()
										}else{
				...
				props: {
					...
					rules: {
						...
						validator(rule, value, callback) {
							...
							if (!value || regexp.test(value)) {
								request(apis.clientsUniq,{email:value,uid:this.$formData.uid},{}).then(res=>{
									if(res?.data?.total==0){
										callback()
									}else{
										callback("该邮箱已经被占用")
