...
export default defineComponent({
	...
	setup(props, context) {
		...
		const computedCurrentDrawerFormProps = computed(() => mergeDeepRight(drawerFormProps[currentAction.value],{ size: appStore.getScreen.isWideScreen ? undefined : '100%'}));
		// 请求注入数据,不受查询面板提交影响
     ...
     */
		...
		function query() {
			...
			proxy.$request(proxy.$apis.clientsFind, mergeDeepRight(searchData.value, injectData.value)).then((res: any) => {
				if (res.code == 200) {
		...
		function save() {
			formRef.value.validate().then((res: any) => {
				proxy.$request(proxy.$apis.clientsCreateOrUpdate, itemData.value).then((res: any) => {
					if (res.code == 200) {
						proxy.$message({
							message: res.msg,
							type: "success",
						...
						// 如果编辑的是当前用户，操作后，直接更新用户信息
						if(userStore.getInfo.uid == itemData.value.uid) {
							userStore.setInfo(itemData.value);
						}
						query();
					} else {
						proxy.$message({
