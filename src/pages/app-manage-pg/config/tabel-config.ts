	...
	// { label: "view", action: "view", size: "small", type: "success", class: "mx-[0px] mr-[5px] last:mr-0" },
	{
		...
		action: (proxy, row) => {
			proxy
				.$confirm({
					title: "操作提示",
					message: `您确定要删除应用 [ ${row.name} ] 吗`,
				})
				.then(() => {
					proxy.$request(proxy.$apis.appDelete, { _id: row._id }, { method:"delete" }).then((res: any) => {
						if (res.code == 200) {
							proxy.$parent.query();
							proxy.$message({
								message: res.msg,
...
export function topOptions(proxy:any):BatchButton[]{
	return [
		{ label: "创建应用", action: (selected, proxy)=>{
				console.log(proxy)
		...
		}, size: "default", type: "primary", class: "mx-[0px] mr-[5px] last:mr-0" },
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要删除所选的 [ ${_ids.length} ] 个应用吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.appDelete, { _id: _ids.join(",") },{method:'delete'}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要失活所选的 [ ${_ids.length} ] 个应用吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.appUpdateMany, { _ids, status: false }, { method:"put"}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要激活所选的 [ ${_ids.length} ] 个应用吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.appUpdateMany, { _ids, status: true }, { method:"put"}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
