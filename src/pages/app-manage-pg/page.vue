...
export default defineComponent({
     ...
     */
		...
		function query() {
			...
			proxy.$request(proxy.$apis.appFind, mergeDeepRight(searchData.value, injectData.value),{method:"POST"}).then((res: any) => {
				if (res.code == 200) {
		...
		function save() {
			formRef.value.validate().then((res: any) => {
				proxy.$request(proxy.$apis.appSave, itemData.value, {method:"POST"}).then((res: any) => {
					if (res.code == 200) {
						proxy.$message({
							message: res.msg,
							type: "success",
