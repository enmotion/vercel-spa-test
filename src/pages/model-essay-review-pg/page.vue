		...
		>
			...
			<div class="flex-col grow-1">
				...
				>
					...
					<div class="flex-col mt-[10px] grow-1 ml-[20px] w-1/3 border rounded border-[#88888820] mb-[20px] p-[20px]">
						<el-scrollbar
							class="flex-col h-0 grow-1"
						>
							...
							<div 
								v-if="itemData.vectorKeyWords?.length>0"	
								class="flex-col mb-[20px]"
							>
								<div class="text-base mb-[10px]">
									内容标签：
								</div>
								<div class="flex-row items-start flex-wrap">
									<div 
										v-for="(item,index) in  itemData.vectorKeyWords"
										:key="item"
										class="flex-col mb-[5px] mr-[5px] hover:bg-s-10 border border-[#88888820] rounded p-[10px] transition-all duration-200 cursor-pointer"
									>
										<p class="text-sm text-gray-500">
											<span class="text-sm text-white mr-[5px]">{{ item }}</span>
										</p>							
									</div>
								</div>
							</div>
     ...
     */
		...
		function save(data:Record<string,any>={}) {
			formRef.value.validate().then((res: any) => {
				isLoading.value = true;
				const request:Record<string,any> = R.pick(
					['_id','uuid','title','content','genre','writingMethods','sync','vectorKeyWords','appreciationGuide','from','status','processingStatus'],
					R.mergeDeepRight(itemData.value, data) as Record<string,any>
				)
				proxy.$request(proxy.$apis.modelEssaySave, request, {method:"POST"}).then((res: any) => {
					if (res.code == 200) {
						proxy.$message({
							message: res.msg,
							type: "success",
