	...
	return [
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要下架所选的 [ ${uuids.length} ] 个范文吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.modelEssayUpdateMany, { uuids, status: false }, { method:"put"}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
		...
		},
		{
			...
			action: (selected, proxy) => {
				...
				proxy
					.$confirm({
						title: "操作警告",
						message: `您确定要上架所选的 [ ${uuids.length} ] 个范文吗`,
					})
					.then(() => {
						proxy.$request(proxy.$apis.modelEssayUpdateMany, { uuids, status: true }, { method:"put"}).then((res: any) => {
							if (res.code == 200) {
								proxy.$parent.query();
								proxy.$message({
									message: res.msg,
	...
	columns: (proxy) => [
      ...
      width: "40px",
			selectable:(row,index)=>{
				...
				return row.super <= userStore.getInfo.super && ![3].includes(row.processingStatus)
			}
		},
		{
			label: "范文标题",
      ...
      invisible:true,
		...
		},
		{
			...
			formatter: oprationFormatter((row, column, value, inxe) => {
				return row.super <= userStore.getInfo.super && row.status ? options.filter(item=>['查看'].includes(item.label)) : options.filter(item=>!['查看'].includes(item.label))
			}, proxy),
		},
	],
};
