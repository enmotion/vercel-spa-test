...
export default defineComponent({
     ...
     */
		...
		function query() {
			...
			proxy.$request(proxy.$apis.usersFind, mergeDeepRight(searchData.value, injectData.value)).then((res: any) => {
				if (res.code == 200) {
		...
		function save() {
			formRef.value.validate().then((res: any) => {
				proxy.$request(proxy.$apis.userCreateOrUpdate, itemData.value).then((res: any) => {
					if (res.code == 200) {
						proxy.$message({
							message: res.msg,
							type: "success",
